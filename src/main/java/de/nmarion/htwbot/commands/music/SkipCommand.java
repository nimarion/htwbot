  ...
  public void execute(SlashCommandEvent event) {
    if (DiscordUtils.isConnected(event)) {
      ...
      if (getBot().getMusicManager().getPlayingTrack(event.getGuild()) != null) {
        final AudioTrackInfo trackInfo = getBot().getMusicManager().getPlayingTrack(event.getGuild()).getInfo();
        final String length;
        ...
        final EmbedBuilder embedBuilder = getEmbed(event).addField(trackInfo.title,
