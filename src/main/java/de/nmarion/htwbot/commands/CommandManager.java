...
public class CommandManager extends ListenerAdapter {
  ...
  public CommandManager(final HtwBot bot) {
    ...
    for (Class<? extends Command> cmdClass : new Reflections("de.nmarion.htwbot.commands")
  ...
  public void onSlashCommand(SlashCommandEvent event) {
    ...
    final Optional<Command> optional = availableCommands.stream()
        .filter(command -> command.getCommand().equalsIgnoreCase(event.getName())).findFirst();
