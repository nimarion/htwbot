...
public class MemberCommand extends Command {
  ...
  public void execute(String[] args, Message message) {
    final EmbedBuilder embedBuilder =
        getEmbed(message.getMember()).setTitle("Mitglieder Statistik");
    message
        .getGuild()
        .loadMembers()
        .onSuccess(
            success -> {
              final Map<Role, Long> roles =
                  success.stream()
                      .filter(
                          member ->
                              PermissionUtil.canInteract(
                                  message.getGuild().getSelfMember(), member))
                      .map(member -> member.getRoles())
                      .flatMap(
                          stream ->
                              stream.stream()
                                  .filter(role -> (!role.isManaged() && role.getName() != "Bot")))
