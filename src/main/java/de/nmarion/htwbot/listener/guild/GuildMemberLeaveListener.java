...
public class GuildMemberLeaveListener extends ListenerAdapter {
    ...
    public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
        if (Configuration.BOT_CHANNEL != null) {
            ...
            embedBuilder.appendDescription(event.getMember().getEffectiveName() + " hat den Server verlassen");
            event.getGuild().getTextChannelById(Configuration.BOT_CHANNEL).sendMessage(embedBuilder.build()).queue();
        }
    }
